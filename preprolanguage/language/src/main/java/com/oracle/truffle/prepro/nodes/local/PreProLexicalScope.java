...
/**
 * PrePro lexical scope. There can be a block scope, or function scope.
 */
public final class PreProLexicalScope {

    private final Node current;
    private final PreProBlockNode block;
    private final PreProBlockNode parentBlock;
    private final RootNode root;
    private PreProLexicalScope parent;
    private Map<String, FrameSlot> varSlots;

    /**
     * Create a new block PrePro lexical scope.
     *
     * @param current     the current node
     * @param block       a nearest block enclosing the current node
     * @param parentBlock a next parent block
     */
    private PreProLexicalScope(Node current, PreProBlockNode block, PreProBlockNode parentBlock) {
    ...
    /**
     * Create a new functional PrePro lexical scope.
     *
     ...
     */
    private PreProLexicalScope(Node current, PreProBlockNode block, RootNode root) {
        this.current = current;
        this.block = block;
    ...
    public static PreProLexicalScope createScope(Node node) {
        PreProBlockNode block = getParentBlock(node);
        if (block == null) {
            // We're in the root.
            ...
            if (block == null) {
                ...
                assert root instanceof PreProEvalRootNode || root instanceof PreProRootNode : "Corrupted SL AST under " + node;
                return new PreProLexicalScope(null, null, (PreProBlockNode) null);
            }
            node = null; // node is above the block
        }
        // Test if there is a parent block. If not, we're in the root scope.
        PreProBlockNode parentBlock = getParentBlock(block);
        if (parentBlock == null) {
            return new PreProLexicalScope(node, block, block.getRootNode());
        } else {
            return new PreProLexicalScope(node, block, parentBlock);
        }
    }

    private static PreProBlockNode getParentBlock(Node node) {
        PreProBlockNode block;
        Node parent = node.getParent();
        // Find a nearest block node.
        while (parent != null && !(parent instanceof PreProBlockNode)) {
            parent = parent.getParent();
        }
        if (parent != null) {
            block = (PreProBlockNode) parent;
        } else {
            block = null;
        }
        return block;
    ...
    private static PreProBlockNode findChildrenBlock(Node node) {
        PreProBlockNode[] blockPtr = new PreProBlockNode[1];
        node.accept(n -> {
            if (n instanceof PreProBlockNode) {
                blockPtr[0] = (PreProBlockNode) n;
                return false;
            } else {
                return true;
            }
    ...
    public PreProLexicalScope findParent() {
        if (parentBlock == null) {
        ...
        if (parent == null) {
            Node node = block;
            PreProBlockNode newBlock = parentBlock;
            // Test if there is a next parent block. If not, we're in the root scope.
            PreProBlockNode newParentBlock = getParentBlock(newBlock);
            if (newParentBlock == null) {
                parent = new PreProLexicalScope(node, newBlock, newBlock.getRootNode());
            } else {
                parent = new PreProLexicalScope(node, newBlock, newParentBlock);
            }
        }
        return parent;
    }
    ...
    private Map<String, FrameSlot> collectVars(Node varsBlock, Node currentNode) {
        ...
        NodeUtil.forEachChild(varsBlock, new NodeVisitor() {
            ...
            public boolean visit(Node node) {
                ...
                if (!(node instanceof PreProBlockNode)) {
                ...
                if (node instanceof PreProWriteLocalVariableNode) {
                    PreProWriteLocalVariableNode wn = (PreProWriteLocalVariableNode) node;
    ...
    private static Map<String, FrameSlot> collectArgs(Node block) {
        ...
        NodeUtil.forEachChild(block, new NodeVisitor() {
            ...
            private PreProWriteLocalVariableNode wn; // The current write node containing a slot

            @Override
            public boolean visit(Node node) {
                // When there is a write node, search for SLReadArgumentNode among its children:
                if (node instanceof PreProWriteLocalVariableNode) {
                    wn = (PreProWriteLocalVariableNode) node;
                    boolean all = NodeUtil.forEachChild(node, this);
                    wn = null;
                    return all;
                } else if (wn != null && (node instanceof PreProReadArgumentNode)) {
                    ...
                    args.put(name, slot);
                    return true;
                } else if (wn == null && (node instanceof PreProStatementNode)) {
