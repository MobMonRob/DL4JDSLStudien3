 ...
 * "evaluating" source code in PrePro.</li>
 * <li>Conversion of arguments to types understood by PrePro. The PrePro source code can be evaluated from a
 * different language, i.e., the caller can be a node from a different language that uses types not
 * understood by PrePro.</li>
 * </ul>
 */
public final class PreProEvalRootNode extends RootNode {
    ...
    public PreProEvalRootNode(PreProLanguage language, RootCallTarget rootFunction, Map<String, RootCallTarget> functions) {
        super(language);
    ...
    public Object execute(VirtualFrame frame) {
        ...
        if (!registered) {
            ...
            lookupContextReference(PreProLanguage.class).get().getFunctionRegistry().register(functions);
            registered = true;
        }
        ...
        for (int i = 0; i < arguments.length; i++) {
            arguments[i] = PreProContext.fromForeignValue(arguments[i]);
        }
