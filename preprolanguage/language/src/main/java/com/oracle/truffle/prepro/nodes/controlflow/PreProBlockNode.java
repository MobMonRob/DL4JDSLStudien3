...
public final class PreProBlockNode extends PreProStatementNode implements BlockNode.ElementExecutor<PreProStatementNode> {
     ...
     */
    @Child
    private BlockNode<PreProStatementNode> block;

    public PreProBlockNode(PreProStatementNode[] bodyNodes) {
        /*
         * Truffle block nodes cannot be empty, that is why we just set the entire block to null if
         * there are no elements. This is good practice as it saves memory.
         */
     ...
     * {@link PreProStatementNode#executeVoid} method of all children to be inlined.
     */
    @Override
    ...
    public List<PreProStatementNode> getStatements() {
        if (block == null) {
     ...
     * startsWith semantics if needed. For PrePro we don't need to pass any argument as we just have
     ...
     */
    public void executeVoid(VirtualFrame frame, PreProStatementNode node, int index, int argument) {
