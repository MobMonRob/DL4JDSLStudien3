...
/**
 * PrePro does not need a sophisticated error checking and reporting mechanism, so all
 * unexpected conditions just abort execution. This exception class is used when we abort
 * from within the PrePro implementation.
 */
public class PreProException extends RuntimeException implements TruffleException {
    ...
    @TruffleBoundary
    public PreProException(String message, Node location) {
        super(message);
    ...
    public static PreProException typeError(Node operation, Object... values) {
        ...
        if (operation != null) {
            NodeInfo nodeInfo = PreProContext.lookupNodeInfo(operation.getClass());
            if (nodeInfo != null) {
        ...
        for (Object value : values) {
            ...
            if (value == null || InteropLibrary.getFactory().getUncached().isNull(value)) {
                result.append(PreProLanguage.toString(value));
            } else {
                result.append(PreProLanguage.getMetaObject(value));
                result.append(" ");
    ...
    @TruffleBoundary
    public static PreProException parameterCountError(Node operation, int index) {
        StringBuilder result = new StringBuilder();
        result.append("Call error");
        if (operation != null) {
            SourceSection ss = operation.getEncapsulatingSourceSection();
            if (ss != null && ss.isAvailable()) {
                result.append(" at ").append(ss.getSource().getName()).append(" line ").append(ss.getStartLine()).append(" col ").append(ss.getStartColumn());
        ...
        result.append(": trying to call function with wrong amount of parameters ");
    ...
    /**
     * Implements the PrePro throw statement.
     */
    @TruffleBoundary
    public static PreProException throW(Node operation, String message) {
        return new PreProException(message, operation);
    }
}
